...
BEGIN
  ...
  IF p_action_type = 'attack' THEN
    ...
    v_damage := GREATEST(1, v_damage); -- Minimum 1 damage
  ...
  ELSIF p_action_type = 'defend' THEN
    -- Defend reduces next attack damage (handled in next turn)
    v_result := jsonb_build_object(
      'action', 'defend',
      'message', 'Defending'
    );
  ELSIF p_action_type = 'special' THEN
    -- Special attack uses venom stat
    v_damage := (v_attacker_spider->>'venom')::integer + 
                floor(random() * 15) - 
                floor((v_defender_spider->>'defense')::integer / 8);
    v_damage := GREATEST(2, v_damage); -- Minimum 2 damage
    v_defender_hp := GREATEST(0, v_defender_hp - v_damage);
