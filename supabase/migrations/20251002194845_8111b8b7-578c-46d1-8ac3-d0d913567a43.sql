-- Phase 6: Battle Resolution & Rewards
-- Add challenge_id to battles table to link battles with challenges
ALTER TABLE public.battles ADD COLUMN challenge_id UUID REFERENCES public.battle_challenges(id);

-- Update the process_battle_turn function to handle battle completion
...
BEGIN
  ...
  IF v_defender_hp <= 0 THEN
    ...
    UPDATE public.battles
    SET winner = CASE 
                   WHEN (team_a->>'userId')::uuid = v_winner_user_id THEN 'TEAM_A'::battle_winner
                   ELSE 'TEAM_B'::battle_winner
                 END,
        is_active = false
    WHERE id = p_battle_id;
    
    -- If this battle is linked to a challenge, resolve it
    IF v_battle.challenge_id IS NOT NULL THEN
      PERFORM public.resolve_battle_challenge(
